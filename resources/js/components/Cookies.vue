...
const guardarPreferencies = () => {
  ...
  if (consentStatus.value.analitiques) {
    setCookie('analytics_cookie', 'accepted', 365); // Set the analytics cookie
  } else {
    deleteCookie('analytics_cookie'); // Remove the analytics cookie
  }

  if (consentStatus.value.marqueting) {
    setCookie('marketing_cookie', 'accepted', 365); // Set the marketing cookie
  } else {
    deleteCookie('marketing_cookie'); // Remove the marketing cookie
  }
...
onMounted(() => {
  ...
  if (guardat) {
    ...
    // Ocultar el banner si todas las cookies necesarias están aceptadas
    if (consentStatus.value.necessaries) {
      mostrarBanner.value = false;
    }
  }
